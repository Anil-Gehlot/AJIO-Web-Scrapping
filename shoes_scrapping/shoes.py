...
for page in range(500):
    ...
    # Set the parameters for the API request for shoes
    params = {
        "fields": "SITE",
        "currentPage": page,  # Specify the desired page number
        "pageSize": 40,
        "format": "json",
        "query": "shoes:relevance",
        "sortBy": "relevance",
        "text": "shoes",
        "advfilter": "true",
        "platform": "Msite",
    ...
    # Send a GET request to the Ajio API for shoes
    ...
    if response.status_code == 200:
        ...
        if 'products' in data:
            ...
            # Iterate through the list of shoe products
            for product in products:
                ...
                # Create a dictionary for each shoe product and add it to the results list
                result = {
                    "Alt Text": alt_text,
        ...
        print(f"API request for shoes failed with status code: {response.status_code}")

    # Save the extracted shoe data to a CSV file
    with open("ajio_shoes_products.csv", "w", newline='', encoding="utf-8") as csvfile:
        fieldnames = ["Alt Text", "Brand Name", "Price (INR)", "MRP (INR)", "Discount (%)", "URL", "Image URLs"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        
        writer.writeheader()
        
        # Write each shoe product's data to the CSV file
        for product in results:
            writer.writerow(product)

print("Shoe data has been saved to ajio_shoes_products.csv")
